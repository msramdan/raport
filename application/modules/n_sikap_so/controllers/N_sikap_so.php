...
class N_sikap_so extends CI_Controller {
    ...
    public function cetak() {
        ...
        $this->d['data_nilai'] = $this->db->query("SELECT 
                                                    ...
                                                    INNER JOIN t_guru_mapel c ON a.id_kelas = c.id
    ...
    public function simpan_nilai() {
        ...
        if (empty($error)) {
            ...
            for ($i = 1; $i < $p['jumlah']; $i++) {
                ...
                $is_wali = 'Y';
                if ($mode_form == "add") {
                    $this->db->query("INSERT INTO t_nilai_sikap_so (tasm,id_kelas,id_siswa,is_wali,selalu,mulai_meningkat) VALUES ('$tasm','$id_kelas','$id_siswa','$is_wali','$selalu','$meningkat')");
                } else if ($mode_form == "edit") {
                    $this->db->query("UPDATE t_nilai_sikap_so SET is_wali = '".$is_wali."', selalu = '".$selalu."', mulai_meningkat = '".$meningkat."' WHERE id_kelas = '".$id_kelas."' AND tasm = '".$tasm."' AND id_siswa = '".$id_siswa."'");
                }
