            ...
            foreach ($siswa as $s) {
                ...
                if (!empty($mapel)) {
                    ...
                    foreach ($mapel as $m) {
                        ...
                        $jml_p += (Int) $isi_p;
                        $jml_k += (Int) $isi_k;
                        $jml += floatval($isi_p) + floatval($isi_k);
                        
                        $html .= '<td class="ctr">'.$isi_p.'</td><td class="ctr">'.$isi_k.'</td>';
                    }
                }
                $peringkat = array_search($jml, $d['peringkat']) + 1;
                if($peringkat == "1" || $peringkat == "2" || $peringkat == "3"){$wr = 'style="background:yellow;font-weight: bold;"';}else{$wr = '';}

                $html .= '<td class="ctr"><b>'.$jml_p.'</td><td class="ctr"><b>'.$jml_k.'</b></td><td class="ctr"><b>'.$jml.'</b></td><td class="ctr"'.$wr.'>'.$peringkat.'</td></tr>';
            }
        }
        $html .= '</tbody></table>';
