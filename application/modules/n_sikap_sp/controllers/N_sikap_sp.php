...
class N_sikap_sp extends CI_Controller {
    ...
    public function cetak() {
        ...
        $this->d['data_nilai'] = $this->db->query("SELECT 
                                                    ...
                                                    INNER JOIN t_walikelas c ON a.id_kelas = c.id_kelas
                                                    WHERE a.id_kelas = '".$this->d['id_kelas']."' AND a.tasm = '".$this->d['tasm']."'")->result_array();
    ...
    public function simpan_nilai() {
        ...
        if ($p['mode_form'] == "add") {
            ...
            for ($i = 1; $i < $p['jumlah']; $i++) {
                ...
                $meningkat = $p['ssp3_'.$i];
                $sql[] = "('".$this->d['tasm']."','".$id_kelas."','".$p['id_siswa_'.$i]."','".$is_wali."', '".$selalu."','".$meningkat."')";
            }
            $this->db->query("INSERT INTO t_nilai_sikap_sp (tasm,id_kelas,id_siswa,is_wali,selalu,mulai_meningkat) VALUES ".implode(",", $sql).";");
        } else if ($p['mode_form'] == "edit") {
            for ($i = 1; $i < $p['jumlah']; $i++) {
                $selalu = $p['ssp1_'.$i]."-".$p['ssp2_'.$i];
                $meningkat = $p['ssp3_'.$i];
                
                $this->db->query("UPDATE t_nilai_sikap_sp SET id_kelas = '".$id_kelas."', selalu = '".$selalu."', mulai_meningkat = '".$meningkat."' WHERE id_kelas = '".$id_kelas."' AND tasm = '".$this->d['tasm']."' AND id_siswa = '".$p['id_siswa_'.$i]."'");
            }
        }
        $d['status'] = "ok";
